...
Bloque *generarBola(Serpiente*,int);
...
int main(){
	...
	Bloque *bloque = generarBola(serpiente,ESCENARIO);
	if(start){
	...
	while(!game_over && start){
			...
			if(direccion == ARRIBA)
				...
				switch(detectarColisiones(serpiente, bloque, ESCENARIO)){
					case COLISION_BOLA:
						...
						Sleep(VELOCIDAD);
						bloque = generarBola(serpiente,ESCENARIO);
						break;
		...
		// moverCursor(COLS+2,2);
		// printf("%d", portalH);
		// moverCursor(COLS+2,3);
		// printf("%d", portalV);
...
Bloque *generarBola(Serpiente*s,int ESCEN){
	Bloque *aux = s->cola;
	...
	Bloque *bola = crearBloque(c,254,0x04); //254 craracter ascii
	imprimirBloque(bola);
	return bola;
}
